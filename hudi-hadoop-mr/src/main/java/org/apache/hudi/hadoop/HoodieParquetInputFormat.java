...
public class HoodieParquetInputFormat extends MapredParquetInputFormat implements Configurable {
  ...
  protected HoodieInputFormatUtils hoodieInputFormatUtils;
  ...
  public FileStatus[] listStatus(JobConf job) throws IOException {
    ...
    if (snapshotPaths.size() > 0) {
      ...
      for (Map.Entry<HoodieTableMetaClient, List<FileStatus>> entry : groupedFileStatus.entrySet()) {
        List<FileStatus> result = hoodieInputFormatUtils.filterFileStatusForSnapshotMode(entry.getKey(), entry.getValue());
        if (result != null) {
          returns.addAll(result);
        }
      }
  ...
  private List<FileStatus> listStatusForIncrementalMode(
    ...
    FileStatus[] fileStatuses = super.listStatus(job);
    return hoodieInputFormatUtils.filterIncrementalFileStatus(tableMetaClient, timeline.get(), fileStatuses, commitsToCheck.get());
  }

  public void setConf(Configuration conf) {
    this.conf = conf;
    hoodieInputFormatUtils = new HoodieInputFormatUtils(this.conf);
  }
