...
public class HoodieParquetRealtimeInputFormat extends HoodieParquetInputFormat implements Configurable {
  ...
  private HoodieRealtimeInputFormatUtils hoodieRealtimeInputFormatUtils = new HoodieRealtimeInputFormatUtils();
  ...
  private static void addRequiredProjectionFields(Configuration configuration) {
    ...
    addProjectionField(configuration, HoodieRecord.PARTITION_PATH_METADATA_FIELD, HoodieRealtimeInputFormatUtils.HOODIE_PARTITION_PATH_COL_POS);
  }
  ...
  public RecordReader<NullWritable, ArrayWritable> getRecordReader(final InputSplit split, final JobConf jobConf,
    ...
    if (jobConf.get(HoodieRealtimeInputFormatUtils.HOODIE_READ_COLUMNS_PROP) == null) {
      synchronized (jobConf) {
        ...
        if (jobConf.get(HoodieRealtimeInputFormatUtils.HOODIE_READ_COLUMNS_PROP) == null) {
          ...
          this.conf = jobConf;
          this.conf.set(HoodieRealtimeInputFormatUtils.HOODIE_READ_COLUMNS_PROP, "true");
        }
      }
    }
