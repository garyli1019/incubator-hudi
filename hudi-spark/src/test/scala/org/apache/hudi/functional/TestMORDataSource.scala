...
class TestMORDataSource {
  ...
  @Test def testMORDataSource() {
    ...
    val records4 = DataSourceTestUtils
      .convertToStringList(newDataGen.generateInserts("004", 100)).toList
    ...
    val records5 = DataSourceTestUtils
      .convertToStringList(newDataGen.generateUpdates("005", 100)).toList
  ...
  def testPayloadToDelete() {
    ...
    val records2 = DataSourceTestUtils.convertToStringList(dataGen.generateUniqueDeleteRecords("002", 50)).toList
    val inputDF2: Dataset[Row] = spark.read.json(spark.sparkContext.parallelize(records2, 2))
    inputDF2.write.format("org.apache.hudi")
      .options(commonOpts)
      .mode(SaveMode.Append)
    ...
    val records3 = DataSourceTestUtils.convertToStringList(dataGen.generateUniqueDeleteRecords("003", 50)).toList
    val inputDF3: Dataset[Row] = spark.read.json(spark.sparkContext.parallelize(records3, 2))
    inputDF3.write.format("org.apache.hudi")
      .options(commonOpts)
      .mode(SaveMode.Append)
